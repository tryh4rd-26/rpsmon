...
pub fn draw(f: &mut Frame, app: &App, area: Rect) {
    ...
    let ram_composite = Line::from([
        ...
        Span::styled(format!("{:.1}G ", used_gb), Style::default().fg(theme::TEXT_VALUE)),
    ].into_iter().chain(ram_bar.spans.into_iter()).collect::<Vec<_>>());
    ...
    let swap_composite = Line::from([
        ...
        Span::styled(format!("{:.1}G/{:.1}G ", swap_used_gb, swap_total_gb), Style::default().fg(theme::TEXT_VALUE)),
    ].into_iter().chain(swap_bar.spans.into_iter()).collect::<Vec<_>>());
    f.render_widget(Paragraph::new(swap_composite), inner[1]);

    // ── Memory Type Bars ──
    let wired = pm.get_mem_wired() * 4096;
    let compressed = pm.get_mem_compressed() * 4096;
    let purgeable = pm.get_mem_purgeable() * 4096;
    let app_mem = pm.get_mem_app();
    let anonymous = pm.get_mem_anonymous() * 4096;
    let file_backed = pm.get_mem_file_backed() * 4096;
    ...
    for (label, val, color) in bar_labels {
        let pct = if total_mem > 0 { (val as f64 / total_mem as f64) * 100.0 } else { 0.0 };
        ...
        let mut line_spans = vec![
            Span::styled(format!(" {} ", label), Style::default().fg(theme::TEXT_LABEL)),
            Span::styled(format!("{:.1}G ", val_gb), Style::default().fg(theme::TEXT_VALUE)),
        ];
    ...
    let mem_canvas = Canvas::default()
        ...
        .paint(|ctx| {
            let history: Vec<(f64, f64)> = app.mem_history.iter().enumerate()
                .map(|(i, &v)| {
            ...
            for &(x, y) in &history {
                if y.abs() > 0.5 {
                    ...
                    for step in 0..steps {
                        ...
                        let y2 = if y >= 0.0 { (sy + 2.0).min(y) } else { -(sy + 2.0).min(y.abs()) };
    ...
    let info_lines = vec![
        Line::from(vec![
            ...
            Span::styled(format!("{:.0}% ", mem_pct), Style::default().fg(theme::mem_color(mem_pct as f32))),
            Span::styled("Avail: ", Style::default().fg(theme::TEXT_LABEL)),
            Span::styled(format!("{:.1}G ", avail_gb), Style::default().fg(theme::TEXT_VALUE)),
            Span::styled("Free: ", Style::default().fg(theme::TEXT_LABEL)),
            Span::styled(format!("{:.1}G ", free_gb), Style::default().fg(theme::TEXT_BRIGHT)),
        ]),
        Line::from(vec![
            Span::styled("Cache: ", Style::default().fg(theme::TEXT_LABEL)),
            Span::styled(format!("{:.0}M ", cache_mb), Style::default().fg(theme::COLOR_BLUE)),
